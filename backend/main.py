...
# NOTE: File parsing imports (PyPDF2, python-docx) are no longer needed here.
# File parsing is now handled by the ingestion-service.
...
# NOTE: File parsing functions (parse_pdf, parse_docx, parse_txt, parse_json, parse_file)
# have been moved to ingestion-service/ingestion_logic.py
# All file uploads are now processed by the ingestion microservice.



# Helper functions

# NOTE: Chunking functions (chunk_by_lines, chunk_by_paragraphs, chunk_by_sentences,
# chunk_by_size, chunk_by_custom_separator, chunk_text, chunk_text_semantic)
# have been moved to ingestion-service/ingestion_logic.py
# All document chunking is now handled by the ingestion microservice.
...
# NOTE: detect_document_type has been moved to ingestion-service/ingestion_logic.py



# NOTE: create_contextual_chunk has been moved to ingestion-service/ingestion_logic.py
...
# NOTE: process_document_async has been removed.
# All document processing is now delegated to the ingestion-service.
...
async def create_document(
    ...
    get_db, ingestion_client, file_storage, microservices_enabled = get_microservices()
    ...
    try:
        ...
        # Still validate locally to fail fast if content is empty
        if not document.content or not document.content.strip():
            raise HTTPException(status_code=400, detail="Content cannot be empty")
        
        # Step 2: Validate chunking parameters (simple validation)
        if document.chunk_size and document.chunk_size < 50:
             raise HTTPException(status_code=400, detail="Chunk size usually should be at least 50 characters")
        
        # Step 3: Create document/task IDs
        document_id = str(uuid.uuid4())
        job_id = str(uuid.uuid4())
        
        # Check if creating a new version (this logic relies on ChromaDB which we still access)
        version = 1
        ...
        if document.create_new_version:
            ...
            # The ingestion service handles marking old versions if we pass create_new_version=True
        ...
        # Step 5: Store content as a file
        db = next(get_db())
        try:
            # Treat text content as a .txt file
            file_content = document.content.encode('utf-8')
            filename = f"{document.metadata.name}.txt"
            
            # Use file storage manager from microservices
        ...
        # Initialize status (ingestion service will update this, but we initialize for immediate feedback)
...
async def upload_document(
    ...
    try:
        ...
        if microservices_enabled:
            ...
            # Microservices are required now
            logger.error("Microservices are not enabled/available")
...
async def update_document(
    ...
    try:
        ...
        # Get existing document to ensure it exists and get content if needed
        ...
        try:
            ...
            # Trigger ingestion (this will handle re-chunking and replacing)
            # Note: Ingestion service typically defaults to creating new chunks.
            # Ideally ingestion service should handle cleanup of old chunks if we are updating same doc ID.
            await ingestion_client.trigger_ingestion(
                job_id=job_id,
                file_id=stored_file.id,
                file_path=str(file_storage.files_dir.parent / stored_file.storage_path),
                ...
                version=int(updated_metadata.get("version", 1)) # Keep version or increment?
            )
